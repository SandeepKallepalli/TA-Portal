...
** How to start?
- extract the downloaded zip.Then inside the TA-PORTAL lies our application . There will be file called run.py , run command =python run.py=
  , this will make the website to be hosted at our local machine.
- To use  the portal go to the link =http://127.0.0.1:8080/= then you will get our main home page .
- The command automatically takes care of the database file creation as we have written it in file =config.py= . 
